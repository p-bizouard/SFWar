...
$vaisseaux = $terrestre;
${'vaisseaux_'.$race_name} = ${'terrestre_'.$race_name};
  ...
  $sqll = "SELECT * FROM sg_".$type." WHERE id='$id_flotte'";	
  ...
  $sqll = "SELECT * FROM sg_planete WHERE id='$id_adversaire'";	
  $query = mysql_query($sqll);
  $donnees = mysql_fetch_array($query);
...
if ($type == "flotte") 
  {
    ...
    if ((puissance_plapla($id_adversaire, $pseudo_ennemi, true) > ((3*puissance_flotte($id_adversaire, $pseudo_ennemi, true)/5) + puissance_flotte($id_adversaire, $pseudo_ennemi, true))) AND (puissance_flotte($id_adversaire, $pseudo_ennemi, true) != 0)) 
      {
        $message_erreur = "Impossible d'attaquer une planete ayant moins 3/5 de votre puissance. (".number_format(puissance_plapla($id_adversaire, $pseudo_ennemi, true), 0, ',', ' ') ."pour votre ennemi contre ". number_format(((3*puissance_plapla($id_adversaire, $pseudo_ennemi, true)/5) + puissance_plapla($id_adversaire, $pseudo_ennemi, true)), 0, ',', ' ')." au maximum pour vous.";
  ...
  {
    if ((puissance_plapla($id_adversaire, $pseudo_ennemi) < (3 * puissance_plapla($id_flotte, $pseudo)/5) +puissance_plapla($id_flotte, $pseudo)) AND (puissance_plapla($id_adversaire, $pseudo_ennemi) != 0)) 
      {
        $message_erreur = "Impossible d'attaquer une planete n'ayant pas au moins les 3/5 de votre puissance (".number_format(puissance_plapla($id_adversaire, $pseudo_ennemi, true), 0, ',', ' ') ."pour votre ennemi contre ". number_format(((3*puissance_plapla($id_adversaire, $pseudo_ennemi, true)/5) + puissance_plapla($id_adversaire, $pseudo_ennemi, true)), 0, ',', ' ')." au maximum pour vous.";
        include ("index/erreur.php");
...
$id_attaquant = $id_flotte;
$id_defenseur = $_POST['adversaire'];


$retour = mysql_query("SELECT COUNT(*) AS nbre_entrees FROM sg_" . $type . "_units WHERE id_" . $type . "='$id_attaquant'  AND unit='terrestre'");
$donnees = mysql_fetch_array($retour);
$nombre_attaquant = $donnees['nbre_entrees'];

$retour = mysql_query("SELECT COUNT(*) AS nbre_entrees FROM sg_planete_units WHERE id_planete='$id_defenseur' AND unit='terrestre'");
$donnees = mysql_fetch_array($retour);
$nombre_defenseur = $donnees['nbre_entrees'];

/////////////////////////////////////////////////////////

$reponse = mysql_query("SELECT * FROM sg_" . $type . " WHERE id='$id_attaquant'");
$donnees = mysql_fetch_array($reponse);
$race_attaquant = race_name_to_id($donnees['race']);



$reponse = mysql_query("SELECT * FROM sg_planete WHERE id='$id_defenseur'");
$donnees = mysql_fetch_array($reponse);
...
$combat_rapport_attaquant = "<br />".'<font color="#6983A3"><b>Votre ' . $type . ' '.$type.' '. $nom_flotte .' attaque la planete ' . $nom_flotte_ennemi.".</b></font><br /><br />";
$combat_rapport_defenseur = "<br />".'<font color="#6983A3"><b>Votre planete '. $nom_flotte_ennemi .' est attaqu&eacute;e par la ' . $type . ' ' . $nom_flotte.".</b></font><br /><br />";
...
if ($nombre_attaquant==0) {
	?>
		<p><font color="#6983A3"><b>Vous devez poss&eacute;der des unit&eacute;s terrestres pour attaquer une planete !</b></font></p>
	<?
} elseif ($nombre_defenseur==0) {// on v&eacute;rifie que la flotte de d&eacute;fense n'est pas vide...

  $combat_rapport_attaquant .= '<font color="#6983A3"><b>La planete adverse n\'avait pas de quoi vous affronter ! La planete '. $nom_flotte_ennemi . ' est a vous!</b></font><br />';
  ...
  assign_planet($id_defenseur,$pseudo);

  echo $combat_rapport_attaquant;

} else {// on g&egrave;re le combat
	$reponse = mysql_query("SELECT * FROM sg_" . $type . "_units WHERE id_" . $type . "='$id_attaquant'  AND `nombre`!='0' AND unit='terrestre'");
  ...
  while ($donnees = mysql_fetch_array($reponse)) {
	...
	$reponse = mysql_query("SELECT * FROM sg_planete_units WHERE id_planete='$id_defenseur' AND `nombre`!='0' AND unit='terrestre'");
...
// TESTs
/*
-v&eacute;rifier nb vaisseau defenseut tue est inf&eacute;rieur au nombre total d\'unit&eacute;s
...
rapport_combat($vie_aleatoire, $degat_aleatoire, $type_vaisseau_attaquant, $type_vaisseau_defenseur, $nombre_vaisseau_attaquant, $nombre_vaisseau_defenseur, $nb_vaisseau_attaque, $nb_vaisseau_defense, $nb_vaisseau_attaque,$nb_vaisseau_defense, $nb_vaisseaux_defenseur_tues, $nb_vaisseaux_attaquant_tues, $nb_vaisseaux_attaquant_restants, $nb_vaisseaux_defenseur_restants) ;
	...
	$nb_vagues = count($type_vaisseau_attaquant);
		...
		for ($i=0; $i < count($nb_vaisseaux_attaquant_restants[$nb_vagues-1]) ; $i++) 
			...
			$puissance_perdue_attaquant += $nb_vaisseaux_perdus_attaquant * $vaisseaux[$type_vaisseau_attaquant[$nb_vagues-1][$i]]['Puissance'];
				...
				mysql_query("UPDATE  sg_" . $type . "_units  SET nombre='".$nb_vaisseaux_attaquant_restants[$nb_vagues-1][$i]."' WHERE id_" . $type . "='$id_attaquant' and unit='terrestre' and type='".$type_vaisseau_attaquant[$nb_vagues-1][$i]."'");
		...
		for ($i=0; $i < count($nb_vaisseaux_defenseur_restants[$nb_vagues-1]) ; $i++) {
			...
			mysql_query("UPDATE  sg_planete_units  SET nombre='".$nb_vaisseaux_defenseur_restants[$nb_vagues-1][$i]."' WHERE id_planete='$id_defenseur' and unit='terrestre' and type='".$type_vaisseau_defenseur[$nb_vagues-1][$i]."'");
	...
	if ($nb_vaisseaux_total_attaquant_restant==0)  {
		...
		mysql_query("DELETE FROM sg_" . $type . " WHERE id='$id_attaquant'");
		mysql_query("DELETE FROM sg_planete_units WHERE id_planete='$id_attaquant'");
		...
		historique($combat_rapport_defenseur . "<br />".'Vous avez contr&eacute; l\'attaque &eacute;nnemie, leurs forces sont reduites a n&eacute;ant ! F&eacute;licitation.<br />', $pseudo_ennemi);
		...
		$combat_rapport_defenseur .= $message . "<br />".'<font color="#6983A3"><b>Vous avez contr&eacute; l\'attaque &eacute;nnemie, leurs forces sont reduites a n&eacute;ant ! F&eacute;licitation.</b></font><br />';
	...
	if ($nb_vaisseaux_total_attaquant_restant != 0 AND $nb_vaisseaux_total_defenseur_restant == 0)  {
		...
		mysql_query("DELETE FROM sg_" . $type . " WHERE id='$id_defenseur'");
		mysql_query("DELETE FROM sg_planete_units WHERE id_planete='$id_defenseur'");
    
		historique($combat_rapport_attaquant . "<br />".'Vous remportez le combat, et d&eacute;truisez les defenses de ' . $nom_flotte_ennemi . ' !<br />', $pseudo);
		historique($combat_rapport_defenseur . "<br />".'Toutes vos unit&eacute;es ont &eacute;t&eacute; d&eacute;truitres. Vos d&eacute;fenses ont faillit, votre planete ' . $nom_flotte_ennemi . ' est perdue.<br />', $pseudo_ennemi);
		
		$combat_rapport_attaquant .= $message . "<br />".'<font color="#6983A3"><b>Vous remportez le combat, et d&eacute;truisez les defenses de ' . $nom_flotte_ennemi . ' !</b></font><br />';
		$combat_rapport_defenseur .= $message . "<br />".'<font color="#6983A3"><b>Toutes vos unit&eacute;es ont &eacute;t&eacute; d&eacute;truitres. Vos d&eacute;fenses ont faillit, votre planete ' . $nom_flotte_ennemi . ' est perdue.</b></font><br />';
	  ...
	  rapport_mail(name_to_id($pseudo_ennemi), $combat_rapport_defenseur);
       ...
       assign_planet($id_defenseur,$pseudo);
	    ...
	    if (verif_mort_perso($pseudo)) {
	      $combat_rapport_attaquant .= "<br />".'Vous fetez la victoire de cette bataille dans votre QG, quand soudaint l\'un de vos officiers vous remette un telegramme provenant de nacelles de sauvetage de la cible ennemie. Sur celui ci est inscrit que dans l\'une d\'entre elles, se trouve le commandant adverse, et qu\'il vous offrira toutes ses ressources si vous le laissez s\'en aller sans detruire son vaisseau. Bien evidement, vous acceptez.'; 
	    }
	}	
  ...
  if (($nb_vaisseaux_total_attaquant_restant !=0 ) AND ($nb_vaisseaux_total_defenseur_restant != 0)) {
    ...
    if ($puissance_perdue_attaquant < $puissance_perdue_defenseur) {
			...
			historique($combat_rapport_attaquant . "<br />".'Vous detruisez plus d\'unit&eacute;s que vous n\'en perdez, vous remportez le combat !<br />', $pseudo);
			...
			$combat_rapport_attaquant .= $message . "<br />".'<font color="#6983A3"><b>Vous detruisez plus d\'unit&eacute;s que vous n\'en perdez, vous remportez le combat !</b></font><br />';
		  ...
		  rapport_mail(name_to_id($pseudo_ennemi), $combat_rapport_defenseur);
			...
			historique($combat_rapport_defenseur . "<br />".'Vous detruisez plus d\'unit&eacute;s que vous n\'en perdez, vous repoussez l\attaque !<br />', $pseudo_ennemi);
			...
			$combat_rapport_defenseur .= $message . "<br />".'<font color="#6983A3"><b>Vous detruisez plus d\'unit&eacute;s que vous n\'en perdez, vous repoussez l\attaque !</b></font><br />';
		  ...
		  rapport_mail(name_to_id($pseudo_ennemi), $combat_rapport_defenseur);
			...
			historique($combat_rapport_attaquant . "<br />".'Vous tuez autant d\'unit&eacute;s ennemis que vous n\'en perdez. Il n\'y a donc ni vainqueur, ni vaincus... pour cette fois.<br />', $pseudo);
			historique($combat_rapport_defenseur . "<br />".'Vous tuez autant d\'unit&eacute;s ennemis que vous n\'en perdez. Il n\'y a donc ni vainqueur, ni vaincus... pour cette fois.<br />', $pseudo_ennemi);
			
			$combat_rapport_attaquant .= $message . "<br />".'<font color="#6983A3"><b>Vous tuez autant d\'unit&eacute;s ennemis que vous n\'en perdez. Il n\'y a donc ni vainqueur, ni vaincus... pour cette fois.</b></font><br />';
			$combat_rapport_defenseur .= $message . "<br />".'<font color="#6983A3"><b>Vous tuez autant d\'unit&eacute;s ennemis que vous n\'en perdez. Il n\'y a donc ni vainqueur, ni vaincus... pour cette fois.</b></font><br />';
